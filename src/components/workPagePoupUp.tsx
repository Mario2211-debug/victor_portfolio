...
export default function WorkPagePopup({ isOpen, onClose, children }) {
  return isOpen ? (
    <motion.div
      className="fixed  flex inset-0 items-center  justify-center backdrop-blur-md z-50"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    >
      <div className="bg-neutral-900 mx-5 bg-opacity-60 rounded-lg py-4 px-6 max-w-lg w-full relative">
        <button
          onClick={onClose}
